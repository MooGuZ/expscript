...
%% load dataset
load(fullfile(datadir, 'mvdot.mat'));
mvdot.nframes = nframes;   
mvdot.canvasSize = framesize;
%% create prevnet
encoderInput = FrameSlicer(10, 'front', 0).appendto(mvdot.data).aheadof(encoder.DI{1});
decoderRefer = FrameReorder('reverse').appendto(encoderInput);
decoderReferFix = Reshaper().appendto(decoderRefer);
predictRefer = FrameSlicer(10, 'back', 0).appendto(mvdot.data);
...
%% create task
task = CustomTask(taskid, taskdir, model, {mvdot, zerogen}, {decoderObj, predictObj}, {}, ...
